...
func (subscriber *Subscriber) processData(msg *gravity_subscriber.Message) error {
	...
	/*
		log.Info("==================== lastSeq: ", msg.Pipeline.LastSeq)
		if subscriber.lastSeq == msg.Pipeline.LastSeq {
			log.Error("duplicate seq ===========================")
		}
		subscriber.lastSeq = msg.Pipeline.LastSeq
	*/
	...
	conn := subscriber.app.GetEventBus().GetJetStreamConnection()
	for _, channel := range channels {

		for {

			_, err := conn.PublishAsync(channel, newPacket)
			if err == nil {
...
func (subscriber *Subscriber) Init() error {
	...
	err = subscriber.subscriber.Register(gravity_subscriber.SubscriberType_Exporter, "jetstream", subscriberID, subscriberName)
	if err != nil {
		return err
	}
...
func (subscriber *Subscriber) Stop() {
	...
	js := subscriber.app.GetEventBus().GetJetStreamConnection()
